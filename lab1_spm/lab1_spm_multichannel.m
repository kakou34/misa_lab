...
for i=1:5
    % Running everything for one case:
    res_seg = segment_brain_tissues( ...
        fullfile(base_data_path, num2str(i), '/T1.nii,1'), ...
        fullfile(base_data_path, num2str(i), '/T2_FLAIR.nii,1'), ...
        settings);
    labels = double(niftiread(fullfile(base_data_path, num2str(i), '/LabelsForTesting.nii')));
    dice_results(i, :) = dice(res_seg , labels);
end
save('dice_multichannel.mat','dice_results');
...
dice_results_reg_MC = zeros(4, 5, 3);
...
for j=1:length(regs)
    for i=1:5
        ...
        settings);
        labels = double(niftiread(fullfile(base_data_path, num2str(i), '/LabelsForTesting.nii')));
        dice_results_reg_MC(j, i, :) = dice(res_seg , labels);
    end
end
save('dice_reg_MC.mat','dice_results_reg_MC');

%%
table1_t1 = zeros(4, 8);
for i=1:3
    table1_t1(1,2*(i-1)+1) = mean(dice_results_reg_MC(1,:,i)); table1_t1(1,2*(i-1)+2) = std(dice_results_reg_MC(1,:,i));
    table1_t1(2,2*(i-1)+1) = mean(dice_results_reg_MC(2,:,i)); table1_t1(2,2*(i-1)+2) = std(dice_results_reg_MC(2,:,i));
    table1_t1(3,2*(i-1)+1) = mean(dice_results_reg_MC(3,:,i)); table1_t1(3,2*(i-1)+2) = std(dice_results_reg_MC(3,:,i));
    table1_t1(4,2*(i-1)+1) = mean(dice_results_reg_MC(4,:,i)); table1_t1(4,2*(i-1)+2) = std(dice_results_reg_MC(4,:,i));
end
table1_t1(1,7) = mean(reshape(dice_results_reg_MC(1,:,:),1,[])); table1_t1(1,8) = std(reshape(dice_results_reg_MC(1,:,:),1,[]));
table1_t1(2,7) = mean(reshape(dice_results_reg_MC(2,:,:),1,[])); table1_t1(2,8) = std(reshape(dice_results_reg_MC(2,:,:),1,[]));
table1_t1(3,7) = mean(reshape(dice_results_reg_MC(3,:,:),1,[])); table1_t1(3,8) = std(reshape(dice_results_reg_MC(3,:,:),1,[]));
table1_t1(4,7) = mean(reshape(dice_results_reg_MC(4,:,:),1,[])); table1_t1(4,8) = std(reshape(dice_results_reg_MC(4,:,:),1,[]));
table1_t1

table2_t1 = zeros(4, 10);
for i=1:5
    table2_t1(1,2*(i-1)+1) = mean(dice_results_reg_MC(1,i,:)); table2_t1(1,2*(i-1)+2) = std(dice_results_reg_MC(1,i,:));
    table2_t1(2,2*(i-1)+1) = mean(dice_results_reg_MC(2,i,:)); table2_t1(2,2*(i-1)+2) = std(dice_results_reg_MC(2,i,:));
    table2_t1(3,2*(i-1)+1) = mean(dice_results_reg_MC(3,i,:)); table2_t1(3,2*(i-1)+2) = std(dice_results_reg_MC(3,i,:));
    table2_t1(4,2*(i-1)+1) = mean(dice_results_reg_MC(4,i,:)); table2_t1(4,2*(i-1)+2) = std(dice_results_reg_MC(4,i,:));
end
table2_t1
